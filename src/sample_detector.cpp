...
int main()
{
	...
	config_v4.file_model_cfg = "../configs/yolov4-obj_hz.cfg";
	config_v4.file_model_weights = "../configs/yolov4-obj_hz_best.weights";
    ...
    cv::String path("/home/linhan/datasets/0910/cam2/*.jpg");
    cv::glob(path, images);
    Timer timer;
    ...
    for(auto image: images)
    {
        ...
        timer.out("detect");
        BatchResult results = batch_res[0];
        std::vector<cv::Rect> rect_res;
        std::cout << "size:" << results.size() << std::endl;
        for(int i = 0; i < results.size(); i++)
        {
            std::cout << "id:" << results[i].id << std::endl;
        }
        for(auto &r: results)
        {
            if (r.id == 0)
            rect_res.push_back(r.rect);
        }
        //display original image
        if(display)
        {
            std::cout << "Raw detections:" << std::endl;
            cv::Mat orignal_img = img.clone();
            for(auto dis_rec: rect_res)
            {
                //disp
                std::cout << " x:" << dis_rec.x << " y:" << dis_rec.y << " w:" << dis_rec.width << "h:" << dis_rec.height << std::endl;
                cv::rectangle(orignal_img, dis_rec, cv::Scalar(255, 0, 0), 2);
            }
        ...
        if (!rect_res.empty())
        {
            AssociateDetectionsToTrackers(rect_res, tracks, matched, unmatched_det);
        }
